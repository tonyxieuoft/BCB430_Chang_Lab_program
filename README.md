...
First, ensure the requirements are met.

Secondly, download the code. This can be achieved by cloning the repository via the following command:
```
...
Thirdly, `cd` into the program directory `NCBI_Gene_Extraction_Pipeline` via the command line.  

Finally, run the main program by calling `python3 main.py`
